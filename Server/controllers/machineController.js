...
const getMachine = async (req, res) => {
  try {
    if (!req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
      return res.status(400).json({ success: false, error: "Invalid machine ID" });
    }
    ...
    const now = new Date();
    machine.isAvailable = now >= new Date(machine.availableFrom) && now <= new Date(machine.availableTo);
...
const createMachine = async (req, res) => {
  try {
    ...
    if (new Date(req.body.availableFrom) >= new Date(req.body.availableTo)) {
      return res.status(400).json({ success: false, error: "Available To date must be after Available From date" });
    }
...
const updateMachine = async (req, res) => {
  try {
    ...
    if (req.body.availableFrom && req.body.availableTo && new Date(req.body.availableFrom) >= new Date(req.body.availableTo)) {
      return res.status(400).json({ success: false, error: "Available To date must be after Available From date" });
    }
